...
metas: deps
	go-bindata -o resource/metas.go -pkg resource $GOPATH/pkg/mod/$(grep -w -F github.com/aliyun/aliyun-openapi-meta go.mod | awk '{print $1"@"$2}')/...
...
build: metas
	go build -trimpath -ldflags "-X 'github.com/aliyun/aliyun-cli/cli.Version=${VERSION}'" -o out/aliyun main/main.go

install: build
	cp out/aliyun /usr/local/bin

build_mac:
	GOOS=darwin GOARCH=amd64 go build -trimpath -ldflags "-X 'github.com/aliyun/aliyun-cli/cli.Version=${VERSION}'" -o out/aliyun main/main.go
...
build_linux:
	GOOS=linux GOARCH=amd64 go build -trimpath -ldflags "-X 'github.com/aliyun/aliyun-cli/cli.Version=${VERSION}'" -o out/aliyun main/main.go
...
build_windows:
	GOOS=windows GOARCH=amd64 go build -trimpath -ldflags "-X 'github.com/aliyun/aliyun-cli/cli.Version=${VERSION}'" -o aliyun.exe main/main.go
